...
public:
    ...
    virtual void register_function(const std::string& name, std::function<Value(const std::vector<std::shared_ptr<ASTNode>>&, Interpreter&)> func) = 0;
    virtual Value call_function(const std::string& name, const std::vector<std::shared_ptr<ASTNode>>& args, Interpreter& interpreter) = 0;
};
...
public:
    ...
    virtual void attach_parser(class Parser* parser) = 0;
};
...
public:
    ...
    virtual void register_token_handler(const std::string& keyword, std::function<std::shared_ptr<ASTNode>(Parser&)> fn) = 0;
